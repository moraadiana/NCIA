...
{
    public partial class EndYearReviewListing : System.Web.UI.Page
    {
                ...
                if (!string.IsNullOrEmpty(appraisalList))
                {
                    ...
                    for (int i = 0; i < appraisalListArr.Length; i++)
                    {
                        ...
                        if (appraisalDetails.Length == 4)
                        {
                            ...
                            htmlStr += $"<td><a href='EndYearReview.aspx?appraisalNo={appraisalNo}&status={status}&query=old'><i class='fa fa-plus-circle text-success'></i><span class='text-success'>Details</span></a></td>";
